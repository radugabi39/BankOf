...
public class UserDAO extends GenericDAO {
	...
	public Customer getCustomerByCNP(String cnp) {
		Query q = em.createQuery(
				"select cust from Customer cust " + " INNER JOIN cust.person pers" + "	where pers.CNP=:cnp");
	...
	public User getUserByCNP(String cnp) {
		Query q = em.createQuery("select u from Customer cust " + " INNER JOIN cust.person pers"
				+ " INNER JOIN cust.user u" + "	where pers.CNP=:cnp");
		q.setParameter("cnp", cnp);
		List<User> toReturn = q.getResultList();
		if (toReturn != null && toReturn.size() > 0) {
			return toReturn.get(0);
	...
	public UserModel getCurrentUserData(String userName) {
		...
		Query q = em.createQuery(
				"select new ro.fmi.bnk.models.UserModel(pers.firstName,pers.lastName,cust.phone,cust.address1,pers.CNP,us.email,ct.name,cnt.name,pers.imageURL) from Customer cust "
						+ " INNER JOIN cust.user us " + " INNER JOIN cust.person pers " + " INNER JOIN cust.city ct "
						+ " INNER JOIN ct.country cnt " + " where us.userName=:userName");
		q.setParameter("userName", userName);
	...
	public UserModel getUserDataByCNP(String cnp) {
		...
		Query q = em.createQuery(
				"select new ro.fmi.bnk.models.UserModel(pers.firstName,pers.lastName,cust.phone,cust.address1,pers.CNP,us.email,ct.name,cnt.name,pers.imageURL) from Customer cust "
						+ " INNER JOIN cust.user us " + " INNER JOIN cust.person pers " + " INNER JOIN cust.city ct "
						+ " INNER JOIN ct.country cnt " + " where pers.CNP=:cnp");
		q.setParameter("cnp", cnp);
		List<UserModel> toReturn = q.getResultList();
		if (toReturn != null && toReturn.size() > 0) {
			return toReturn.get(0);
	...
	public String getProfileImage(String userName) {
		Query q = em.createQuery("select pers.imageURL from Customer cust " + " inner join cust.person pers"
	...
	public Person getPersonByUserName(String userName) {
		Query q = em.createQuery("select pers from Customer cust " + " inner join cust.person pers"
				+ " inner join cust.user u" + " where u.userName=:userName");
		q.setParameter("userName", userName);
	...
	public Boolean checkIfUserIsEmployee(String userName) {
		Query q = em.createQuery(
				"select emp.id from Employee emp" + "	INNER JOIN emp.user u " + " where u.userName=:userName OR u.email=:userName");
		q.setParameter("userName", userName);
